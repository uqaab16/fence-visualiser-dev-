            ...
            <g transform={`translate(${globalOffset.x}, ${globalOffset.y})`}>
              ...
              {(() => {
                ...
                if (material === 'slat_fencing') {
                  ...
                  // 65mm: ~28 slats at 1800mm; 90mm: ~17 slats at 1800mm (gap halved vs original)
                  const slatBodyH = isChunky ? 1.30 : 0.68;
                  const origSlatTotal = Math.max(
                    isChunky
                      ? Math.round(baseVh * 0.40) + 1
                      : Math.round(baseVh * 0.75) - 4,
                    4
                  );
                  const origPitch = baseVh / origSlatTotal;
                  const origGap = origPitch - slatBodyH;
                  const slatGap = origGap / 2;              // halved gap at every height
                  const pitch = slatBodyH + slatGap;        // tighter pitch
                ...
                // ── POST & RAIL GATE RENDER ─────────────────────────────────────────
                if (seg.gateStyle?.startsWith('pr-')) {
                  const prIsRed = color.name === 'Reddish-Brown';
                  const prTexHref   = prIsRed ? '/pr-red-texture.jpg' : '/pr-tan-texture.jpg';
                  const prTexFilter = prIsRed ? undefined : 'url(#pr-tan-adjust)';
                  const prStainDark = prIsRed ? '#2e1108' : '#6b3e18';
                  const prStainMid  = prIsRed ? '#4e2219' : '#a0682e';

                  // Gate width in SVG units (x-axis)
                  const gateW = gx2 - gx1;
                  // Gate height in SVG units — use average of start/end visual heights
                  const gateH = (vhStart + vhEnd) / 2;

                  // Draw one horizontal timber rail as a trapezoid (matches fence rail style)
                  const drawPRRail = (hFrac: number, key: string) => {
                    const railThickFrac = 0.10; // same ratio as fence (railThickness = vh * 0.10)
                    const rThickStart = vhStart * railThickFrac * scaleG1;
                    const rThickEnd   = vhEnd   * railThickFrac * scaleG2;
                    const ax = gx1, ay = gy1 - vhStart * hFrac;
                    const bx = gx2, by = gy2 - vhEnd   * hFrac;
                    const rLeft  = Math.min(ax, bx);
                    const rTop   = Math.min(ay - rThickStart, by - rThickEnd);
                    const rRight = Math.max(ax, bx);
                    const rBot   = Math.max(ay, by);
                    const clipId = `pr-gate-clip-${seg.id}-${key}`;
                    const path   = `M ${ax} ${ay} L ${bx} ${by} L ${bx} ${by - rThickEnd} L ${ax} ${ay - rThickStart} Z`;
                    return (
                      <g key={key}>
                        <defs><clipPath id={clipId}><path d={path} /></clipPath></defs>
                        <image href={prTexHref} x={rLeft} y={rTop} width={rRight - rLeft} height={rBot - rTop}
                          preserveAspectRatio="xMidYMid slice" clipPath={`url(#${clipId})`} filter={prTexFilter} />
                        <path d={path} fill="none" stroke={prStainDark} strokeWidth="0.06" />
                      </g>
                    );
                  };

                  // Draw one diagonal brace as a trapezoid (same thickness as a rail)
                  // fromLeft=true: top-left→bottom-right; false: top-right→bottom-left
                  const drawPRBrace = (fromLeft: boolean, key: string) => {
                    const railThickFrac = 0.10;
                    // Brace runs full height: from top of gate to bottom
                    // We draw it as a line from (gx1, gy1-vhStart) to (gx2, gy2) or mirrored
                    const ax = fromLeft ? gx1 : gx2;
                    const ay = fromLeft ? gy1 - vhStart * scaleG1 : gy2 - vhEnd * scaleG2;
                    const bx = fromLeft ? gx2 : gx1;
                    const by = fromLeft ? gy2 : gy1;
                    // Thickness perpendicular to brace — approximate with rail thickness at midpoint
                    const midThick = ((vhStart * scaleG1 + vhEnd * scaleG2) / 2) * railThickFrac;
                    // Perpendicular offset vector (normalised)
                    const dx = bx - ax, dy = by - ay;
                    const len = Math.hypot(dx, dy);
                    const nx = -dy / len * midThick * 0.5;
                    const ny =  dx / len * midThick * 0.5;
                    const path = `M ${ax + nx} ${ay + ny} L ${bx + nx} ${by + ny} L ${bx - nx} ${by - ny} L ${ax - nx} ${ay - ny} Z`;
                    const rLeft  = Math.min(ax, bx) - Math.abs(nx);
                    const rTop   = Math.min(ay, by) - Math.abs(ny);
                    const rRight = Math.max(ax, bx) + Math.abs(nx);
                    const rBot   = Math.max(ay, by) + Math.abs(ny);
                    const clipId = `pr-gate-brace-${seg.id}-${key}`;
                    return (
                      <g key={key}>
                        <defs><clipPath id={clipId}><path d={path} /></clipPath></defs>
                        <image href={prTexHref} x={rLeft} y={rTop} width={rRight - rLeft} height={rBot - rTop}
                  ...
                  // Draw one timber gate post (matches renderTimberPost in the P&R panel)
                  const drawPRPost = (px_: number, py_: number, scale_: number, key: string) => {
                    const vh_ = getVisualFenceHeight() * scale_;
                    const pw_ = vh_ * 0.075;
                    const capH_ = 0.22 * scale_;
                    const left_   = px_ - pw_ / 2;
                    const top_    = py_ - vh_;
                    const bottom_ = py_ + 0.3;
                    const clipId  = `pr-gate-post-${seg.id}-${key}`;
                    return (
                      <g key={key} filter="url(#pr-shadow)">
                        <defs>
                          <clipPath id={clipId}>
                            <path d={`M ${left_} ${bottom_} L ${left_} ${top_} L ${px_ + pw_/2} ${top_} L ${px_ + pw_/2} ${bottom_} Z`} />
                          </clipPath>
                        </defs>
                        <ellipse cx={px_} cy={py_ + 0.22} rx={pw_ * 0.85} ry="0.18" fill="#000" opacity="0.38" />
                        <image href={prTexHref} x={left_} y={top_} width={pw_} height={bottom_ - top_}
                          preserveAspectRatio="xMidYMid slice" clipPath={`url(#${clipId})`} filter={prTexFilter} />
                        <path d={`M ${left_} ${bottom_} L ${left_} ${top_} L ${px_ + pw_/2} ${top_} L ${px_ + pw_/2} ${bottom_} Z`}
                          fill="none" stroke={prStainDark} strokeWidth="0.08" />
                        {/* Chamfered pyramid cap */}
                        <path d={`M ${left_ - 0.06} ${top_} L ${px_ - pw_*0.12} ${top_ - capH_*0.65} L ${px_} ${top_ - capH_} L ${px_ + pw_*0.12} ${top_ - capH_*0.65} L ${px_ + pw_/2 + 0.06} ${top_} Z`}
                          fill={prStainMid} stroke={prStainDark} strokeWidth="0.04" />
                        {/* Bolt holes */}
                        {[0.25, 0.5, 0.75].map(f => (
                          <circle key={f} cx={px_} cy={top_ + (bottom_ - top_) * f} r={0.07 * scale_} fill={prStainDark} opacity="0.6" />
                        ))}
                      </g>
                    );
                  };

                  // Rail positions per style
                  const prRailPositions: number[] =
                    seg.gateStyle === 'pr-1rail'     ? [0.92]
                    : seg.gateStyle === 'pr-2rail'   ? [0.88, 0.52]
                    : seg.gateStyle === 'pr-3rail'   ? [0.88, 0.52, 0.14]
                    : seg.gateStyle === 'pr-4rail'   ? [0.88, 0.65, 0.42, 0.14]
                    : /* pr-crossbuck */               [0.88, 0.08];

                  return (
                    <g
                      key={`gate-${seg.id}`}
                      className="gate-overlay pointer-events-auto cursor-grab active:cursor-grabbing"
                      onPointerDown={(e) => handlePointerDownGate(e, seg.id, 'move')}
                    >
                      {/* Selection aura */}
                      {isSelected && (
                        <polygon
                          points={`${gx1 - 0.6},${gy1 + 0.4} ${gx2 + 0.6},${gy2 + 0.4} ${gx2 + 0.6},${gy2 - ghtEnd - 0.4} ${gx1 - 0.6},${gy1 - ghtStart - 0.4}`}
                          fill="rgba(20,184,166,0.08)" stroke="#14b8a6" strokeWidth="0.22" strokeDasharray="0.8 0.8"
                          className="pointer-events-none animate-[pulse_2s_infinite_ease-in-out]"
                        />
                      )}

                      {/* Horizontal timber rails */}
                      {prRailPositions.map((hf, i) => drawPRRail(hf, `rail-${i}`))}

                      {/* Diagonal brace(s) — top-left→bottom-right for all, plus mirrored for crossbuck */}
                      {drawPRBrace(true, 'brace-main')}
                      {seg.gateStyle === 'pr-crossbuck' && drawPRBrace(false, 'brace-cross')}

                      {/* Timber gate posts */}
                      {drawPRPost(gx1, gy1, scaleG1, 'post-l')}
                      {drawPRPost(gx2, gy2, scaleG2, 'post-r')}

                      {/* Label + delete bubble (same pattern as generic gates) */}
                      <g transform={`translate(${(gx1 + gx2) / 2}, ${(gy1 + gy2) / 2 - (ghtStart + ghtEnd) / 4 - 2.2})`}>
                        ...
                        <text x={isSelected ? -1.0 : 0} y={0.35} fill="#ffffff" fontSize="0.92" fontFamily="sans-serif" fontWeight="bold" textAnchor="middle">
                          Single Gate
                        </text>
                        {isSelected && (
                          <g transform="translate(2.7, 0)" className="cursor-pointer pointer-events-auto hover:opacity-80"
                            onPointerDown={(e) => { e.stopPropagation(); deleteSelectedSegment(); }}>
